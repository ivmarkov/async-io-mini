...
//! The purpose of this thread is to wait for I/O events reported by OS, and then
...
impl<T: AsFd + AsRawFd + From<OwnedFd>> TryFrom<OwnedFd> for Async<T> {
    type Error = io::Error;
    ...
    T: AsFd + AsRawFd,
{
    ...
    pub fn into_inner(mut self) -> io::Result<T> {
        VFS_REACTOR.deregister(self.as_raw_fd())?;
    ...
    pub fn poll_readable(&self, cx: &mut Context<'_>) -> Poll<io::Result<()>> {
        if VFS_REACTOR.fetch(self.as_raw_fd(), Event::Read)? {
            Poll::Ready(Ok(()))
        } else {
            VFS_REACTOR.set(self.as_raw_fd(), Event::Read, cx.waker())?;
    ...
    pub fn poll_writable(&self, cx: &mut Context<'_>) -> Poll<io::Result<()>> {
        if VFS_REACTOR.fetch(self.as_raw_fd(), Event::Write)? {
            Poll::Ready(Ok(()))
        } else {
            VFS_REACTOR.set(self.as_raw_fd(), Event::Write, cx.waker())?;
    ...
    pub async fn read_with<R>(&self, op: impl FnMut(&T) -> io::Result<R>) -> io::Result<R> {
        VFS_REACTOR.fetch(self.as_raw_fd(), Event::Read)?;
    ...
    pub async fn write_with<R>(&self, op: impl FnMut(&T) -> io::Result<R>) -> io::Result<R> {
        VFS_REACTOR.fetch(self.as_raw_fd(), Event::Write)?;
